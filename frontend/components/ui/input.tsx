import * as React from "react"

import { cn } from "@/lib/utils"

function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  // Check if className contains a custom background class
  const classNameStr = className?.toString() || "";
  // Check for bg- patterns but exclude file: and selection: prefixes
  const hasCustomBg = 
    classNameStr.includes("!bg-") ||
    classNameStr.includes("bg-white") ||
    classNameStr.includes("bg-[") ||
    (classNameStr.includes("bg-") && !classNameStr.includes("file:bg-") && !classNameStr.includes("selection:bg-"));
  
  return (
    <input
      type={type}
      data-slot="input"
      className={cn(
        "file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground border-input h-9 w-full min-w-0 rounded-md border px-3 py-1 text-base shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
        // Only apply default backgrounds if no custom background is provided
        !hasCustomBg && "bg-transparent",
        "focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]",
        "aria-invalid:ring-destructive/20 aria-invalid:border-destructive",
        className
      )}
      {...props}
    />
  )
}

export { Input }
